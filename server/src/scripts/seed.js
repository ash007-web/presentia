...
const seedData = async () => {
  try {
    ...
    console.log('✅ Cycle 1 created');

    // 2. SETTINGS
    await Settings.deleteMany({}); // Delete existing settings to ensure clean defaults
    await Settings.create({
      singletonKey: 'GLOBAL_SETTINGS',
      defaultDuration: 120, // 2 minutes
      currentCycle: cycle1._id,
      animationMode: 'full',
      bellEnabled: true,
    });
    ...
    for (const day of days) {
      ...
      await Timetable.create({ day, periods });
    }
    ...
    const studentsArray = rosterData.map(student => ({
      ...student,
      title: "",
      status: "Pending"
    }));
    ...
    console.log(`✅ ${studentsArray.length} Real Students inserted from class roster`);
